// V3: 511 peak is fit with Gauss + Errf
//     Fit windows are adapted for the data with new shaping times
...
void GetPeakValue_v3()
{
  const int nFilesC = 7;
  const int nFilesA = 6;
  const int N =  nFilesC + nFilesA;

  string fnamesC[nFilesC] = {"2417","2418","2424","2426","2434","2447","2448"};
  string fnamesA[nFilesA] = {"2421","2422","2423","2431","2432","2433"};
  ...
  TF1 *fitLP = new TF1("fitLP",fitFunction,200,600,4);
  TF1 *fitUP = new TF1("fitUP","gaus",1450,1800);
  ...
  for (int i = 0; i < nFilesC; i++) {
     string name = "../analysis/290412/" + fnamesC[i] + ".root";
     ...
     // fill histograms
     tLP->Draw("fecclLP>>hLP");
     tUP->Draw("fecclUP>>hUP");

     // fit the peaks
     hLP->Fit("fitLP","rq");
     hUP->Fit("fitUP","rq");

     cout << fnamesC[i] << "\t" << fitLP->GetParameter(2) << " +- " << fitLP->GetParError(2) << "\t" << fitUP->GetParameter(1) << " +- " << fitUP->GetParError(1) << endl;

     // set data points
     gr1C->SetPoint(i,atoi(fnamesC[i].c_str()),fitLP->GetParameter(2));
     gr2C->SetPoint(i,atoi(fnamesC[i].c_str()),fitUP->GetParameter(1));

     // set errors
     gr1C->SetPointError(i,0,fitLP->GetParError(2));
     gr2C->SetPointError(i,0,fitUP->GetParError(1));

     Peak1Mean[count] = fitLP->GetParameter(2);
  ...
  for (int i = 0; i < nFilesA; i++) {
     string name = "../analysis/290412/" + fnamesA[i] + ".root";
     TFile *f = new TFile(name.c_str(),"READ");
     ...
     cout << fnamesA[i] << "\t" << fitLP->GetParameter(2) << " +- " << fitLP->GetParError(2) << "\t" << fitUP->GetParameter(1) << " +- " << fitUP->GetParError(1) << endl;

     // set data points
     gr1A->SetPoint(i,atoi(fnamesA[i].c_str()),fitLP->GetParameter(2));
     gr2A->SetPoint(i,atoi(fnamesA[i].c_str()),fitUP->GetParameter(1));

     // set errors
     gr1A->SetPointError(i,0,fitLP->GetParError(2));
